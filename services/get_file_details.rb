...
class GetFileDetails
  def self.call(current_uid:, auth_token:, course_id:, folder_id:, file_id:)
    response = HTTP.get("#{ENV['API_HOST']}/accounts/#{current_uid}/#{course_id}/folders/#{folder_id}/files/#{file_id}/")
    # puts response.parse
  ...
  def self.download_file(file_data)
    ...
    enc = file_data['data']['file']['data']['ori_document']
    plain = Base64.strict_decode64(enc)
